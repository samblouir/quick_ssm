...
def aggregator_combine_rev(aR, xR, aL, xL):
    ...
    The function as written `aggregator_combine_rev(aR, xR, aL, xL)` computes:
...
def ssm_local_backward_kernel(
    ...
    for i in tl.static_range(BLOCK_L):
        ...
        if pos >= t_start: # Process only valid indices within the block
            ...
            # aggregator_combine_rev(aR, xR, aL, xL) computes aL*aR, aL*xR + xL
            # This represents applying the future transform R, then the current step L.
            a_new, x_new = aggregator_combine_rev(a_run, x_run, a_next, c_i * dY_i)
...
def ssm_backward_carry_scan_kernel(
    ...
    # `aggregator_combine_rev` defines how to combine backward transforms
    # The scan computes the cumulative transform from each block to the end
    a_out, x_out = tl.associative_scan(
        (a_in, x_in), axis=0, combine_fn=aggregator_combine_rev, reverse=True
    )
...
def ssm_bwd_apply_kernel(
    ...
    for i in tl.static_range(BLOCK_L):
        ...
        if pos >= t_start: # Boundary check
            ...
            # Using aggregator_combine_rev(aR, xR, aL, xL) -> (aL*aR, aL*xR + xL)
            # We need combine(prefix, local) => R=prefix=(a_pref, x_pref), L=local=(a_loc, x_loc)
            _, dH = aggregator_combine_rev(a_pref, x_pref, a_loc, x_loc)
