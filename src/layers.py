...
class SSM(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.u_proj = nn.LinearProjection(hidden_size, state_size_dim, **kwargs)
        self.gate_proj = nn.LinearProjection(hidden_size, state_size_dim, **kwargs)
    ...
    def forward(
        ...
        **kwargs,
        ...
        x_elements = self.u_proj(x)
        sidegate_elements = torch.sigmoid(self.sidegate_proj(x))
        ...
        # --- Core Scan Operation ---
        # Map SSM inputs to scan function arguments:
        # scan(a, x, b, c, ...) corresponds to scan(A, u, b, gate, ...)
        # where:
        #   a = State transition coefficient (A)
        #   x = Input sequence (u)
        #   b = Input scaling factor (b)
        #   c = Output scaling factor (gate)
