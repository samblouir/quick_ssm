...
def ssm_backward_carry_scan_kernel(
    ...
    a_out, x_out = tl.associative_scan(
        (a_in, x_in), 0, aggregator_combine, reverse=True
    )
...
def block_scan_backward_3d(
    ...
    B, L, D = x.shape
    a, b, c, x, h, grad_y = ensure_contiguous_fp32(
        a, b, c, x, h, grad_y, device=x.device
    )
    ...
    ssm_local_backward_kernel[grid1](
        a, c, grad_y, loc_a, loc_x, carry_flat, B, L, D, BLOCK_L
    )
