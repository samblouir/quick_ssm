...
def ssm_bwd_apply_kernel(
    ...
    for i in tl.static_range(BLOCK_L):
        ...
        if pos >= t_start:
            ...
            dY_val = tl.load(dY_ptr + off)
            tl.store(dc_ptr + off, dY_val * h_val)

            b_val = tl.load(b_ptr + off)
            tl.store(dx_ptr + off, b_val * dH)
            ...
            tl.store(db_ptr + off, x_val * dH)


def block_scan_backward_3d(
    a, b, c, x, h, grad_y, BLOCK_L=256
) -> Tuple[torch.Tensor, ...]:
    """
    ...
    if L == 0:
        return (torch.empty_like(x), torch.empty_like(a), torch.empty_like(b), torch.empty_like(c))
    ...
    ssm_local_backward_kernel[grid](
        a, c, grad_y, loc_a, loc_x, carry_flat, B, L, D, BLOCK_L, N_BLOCKS=n_blocks
    )
    ssm_backward_carry_scan_kernel[(n_rows,)](carry_flat, n_rows, n_blocks)
    ssm_bwd_apply_kernel[grid](
        loc_a, loc_x, carry_flat, dx, da, db, dc, x, a, b, h, grad_y, B, L, D, BLOCK_L, N_BLOCKS=n_blocks
    )
    return dx, da, db, dc
